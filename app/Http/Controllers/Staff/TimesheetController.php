...
class TimesheetController extends Controller
{
    ...
    public function filter(\Illuminate\Http\Request $request)
    {
        ...
        {
            return Redirect::to('staff/timesheet');
        }
        else
        {
		    $now = Carbon::now()->format('Y-m-d');
            ...
            $employees = Timesheet::where(function($query) use ($request, $date_start, $date_end)
                ...
                })->orderBy('id', 'desc')->where('user_id', Auth::user()->id)->paginate(7);
            ...
            return view('staff.timesheet')->with(['employees' => $employees,'count' => $count, 'date_start' => $date_start, 'date_end' => $date_end,'totalcount' => $totalcount, 'time_in' => $time_in, 'time_out' => $time_out]);  
        }   
    }
